...
#  Model browser — curated list, HuggingFace, Ollama, LM Studio
# ============================================================
...
class ModelsPanel(ctk.CTkFrame):
    ...
    def _build_ollama(self):
        ...
        ctk.CTkLabel(
            hdr,
            text="Browse Ollama Library models and copy the pull command.",
            font=("Arial", 12),
            text_color=T["text_secondary"],
        ).pack(side="left", anchor="w")
        ...
        self._ollama_status.pack(anchor="w", padx=20, pady=(0, 4))

        # Results scroll area
        self._ollama_scroll = ctk.CTkScrollableFrame(
            self._content, fg_color="transparent")
        self._ollama_scroll.pack(
    ...
    def _fetch_ollama_models(self):
        T = self.theme
        for w in self._ollama_scroll.winfo_children():
            w.destroy()
        self._ollama_status.configure(text="Fetching models from ollama.com…")
        ...
        def _scrape():
            try:
                ...
                self.after(0, lambda: self._render_ollama(unique))
                self.after(0, lambda: self._ollama_status.configure(
                    text=f"Loaded {len(unique)} models from ollama.com"))
            except Exception as e:
                self.after(0, lambda: self._ollama_status.configure(
                    text=f"❌  Could not fetch models: {e}"))

        threading.Thread(target=_scrape, daemon=True).start()

    def _render_ollama(self, model_names: list):
        T = self.theme
        for w in self._ollama_scroll.winfo_children():
            w.destroy()
        ...
        for name in model_names:
            row = ctk.CTkFrame(
                self._ollama_scroll, corner_radius=10,
                fg_color=T["bg_card"])
            row.pack(fill="x", pady=4, padx=4)
            ...
            ctk.CTkButton(
                ...
                hover_color=T["bg_hover"],
                text_color=T["text_secondary"],
                command=lambda n=name: self._copy_ollama_cmd(n),
            ).pack(side="right", padx=12, pady=10)

    def _copy_ollama_cmd(self, name: str):
        cmd = f"ollama pull {name}"
        try:
            import pyperclip
            pyperclip.copy(cmd)
        except Exception:
    ...
    def _build_lmstudio(self):
        ...
        card = ctk.CTkFrame(outer, corner_radius=14, fg_color=T["bg_card"])
        ...
        ctk.CTkLabel(
            card,
            text=(
                "LM Studio uses models in GGUF format — the same format "
                "supported here.\n\n"
                "To download models for LM Studio:\n"
                "  1. Use the HuggingFace tab above to download any GGUF model.\n"
                "  2. Or visit lmstudio.ai to browse and download models directly.\n\n"
                "Downloaded models are saved to the ./models folder and can be\n"
                "loaded by either LM Studio or FreedomForge AI."
            ),
            font=("Arial", 12),
            text_color=T["text_secondary"],
            justify="left",
